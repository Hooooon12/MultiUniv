...
#        self.symmetrizeSystVsEta()
        ...
        #hname  = 'h2_scaleFactorsEGamma' #JH : I commented this out to avoid "TROOT::Append:0: RuntimeWarning: Replacing existing TH1: h2_scaleFactorsEGamma (Potential memory leak)."
        if onlyError >= 0:
...
#        self.symmetrizeSystVsEta()
        ...
        for ptBin in self.effList.keys():
	    #if ptBin[0] == 10 : continue #JH for showing
            for etaBin in self.effList[ptBin].keys():
		#if etaBin[0] == -1.57 or etaBin[0] == 1.44 : continue #JH 
                if etaBin[0] >= 0 and etaBin[1] > 0:
                    etaBinPlus  = etaBin
                    etaBinMinus = (-etaBin[1],-etaBin[0])
